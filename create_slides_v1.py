"""
AI基礎講義_スライド_v1 生成スクリプト
- カラーテーマ A（ネイビー × ブルー）
- スライドサイズ 4:3（10.0" × 7.5"）
- パターン1: AI基礎講義（90分）全17枚
"""
...
# カラーパレット（テーマA）
# ============================================================
C_PRIMARY  = RGBColor(0x1A, 0x2E, 0x4A)  # ネイビー
C_ACCENT   = RGBColor(0x00, 0x8C, 0xD7)  # アクセントブルー
C_BG       = RGBColor(0xF0, 0xF6, 0xFC)  # 薄青背景
C_WHITE    = RGBColor(0xFF, 0xFF, 0xFF)
C_GRAY     = RGBColor(0x6B, 0x7A, 0x8D)
C_PH       = RGBColor(0xCC, 0xCC, 0xCC)  # プレースホルダー

# 4:3 サイズ
W = Inches(10.0)
H = Inches(7.5)

FILENAME = "AI基礎講義_スライド_v1"
...
def add_bullet_para(tf, text, size=14, bold=False, color=C_PRIMARY,
                    indent=0, marker="▶  "):
...
# スライド生成関数
# ============================================================
def make_title_slide(prs, title, subtitle, label):
...
def make_section_divider(prs, num, title, subtitle=""):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
...
def make_content_slide(prs, title, bullets=None,
                       ph=False, ph_label="※ここに内容を入力", note=""):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg(slide, C_BG)
    header(slide, title)
    ...
    if ph:
        ...
        add_ph_box(slide, Inches(0.4), ph_y, Inches(9.2), ph_h, ph_label)
...
def make_two_col_slide(prs, title,
    ...
    add_text(slide, left_title,
             lx + Inches(0.08), Inches(1.2), col_w - Inches(0.16), Inches(0.38),
             size=13, bold=True, color=C_WHITE)
    # 右ヘッダー
    add_rect(slide, rx, Inches(1.18), col_w, Inches(0.42), C_PRIMARY)
    add_text(slide, right_title,
             rx + Inches(0.08), Inches(1.2), col_w - Inches(0.16), Inches(0.38),
             size=13, bold=True, color=C_WHITE)
...
def build():
    ...
    # ── Slide 2: アジェンダ ───────────────────────────────────
    make_agenda_slide(prs, "本日のアジェンダ", [
    ...
    # ── Slide 3: 今日のゴール ─────────────────────────────────
    make_content_slide(prs, "今日終わったら何が変わるか",
        bullets=[
    ...
    # ── Slide 4: AI現在地確認 ─────────────────────────────────
    make_content_slide(prs, "あなたの今のAI使用状況は？",
        bullets=[
    ...
    # ── Slide 5: Section 1 区切り ─────────────────────────────
    make_section_divider(prs, 1,
        "生成AIとは何か",
        "LLMの仕組みと限界を平易に理解する")

    # ── Slide 6: LLMの仕組み ─────────────────────────────────
    make_content_slide(prs, "そもそもAIとは何か：LLMの仕組み",
        bullets=[
            "LLM（大規模言語モデル）とは",
            "どうやって「文章を生成」しているか",
            "なぜ「賢く見える」のか",
    ...
    # ── Slide 7: なぜ間違えるのか ────────────────────────────
    make_content_slide(prs, "なぜAIは間違えるのか：ハルシネーション",
        bullets=[
            "「確率的な次の単語予測」という本質",
    ...
    # ── Slide 10: Section 2 区切り ────────────────────────────
    make_section_divider(prs, 2,
        "AIツール実践",
        "プロンプト設計＋業務ユースケース（40分）")
    ...
    # ── Slide 12: ライブデモ①議事録 ─────────────────────────
    make_content_slide(prs, "ライブデモ①：議事録の自動要約",
        bullets=[
    ...
    # ── Slide 13: ライブデモ②メール ─────────────────────────
    make_content_slide(prs, "ライブデモ②：メール文案の生成",
        bullets=[
    ...
    # ── Slide 15: Section 3 区切り ────────────────────────────
    make_section_divider(prs, 3,
        "社会・産業へのAI浸透",
        "業種別事例と「他社はここまでやっている」")

    # ── Slide 16: 業種別事例 ─────────────────────────────────
    make_content_slide(prs, "業種別 AI導入事例",
        bullets=[
            "## 事例①　【受講者企業に近い業種】",
            "※事例内容・効果をここに記載",
            "## 事例②　【受講者企業に近い業種】",
    ...
    # ── Slide 17: Section 4 区切り ────────────────────────────
    make_section_divider(prs, 4,
        "リスクと注意点",
        "安全に使うための最低限の知識")

    # ── Slide 18: リスクと注意点 ─────────────────────────────
    make_content_slide(prs, "AIを使う上でのリスクと注意点",
        bullets=[
